...
export default function Number(props: any) {
  ...
  const handleDropdownValue = (e: any) => {
    if (e.value == "Đang chờ") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.stateFilter === "Đang chờ"
      );
      setData(filterdata);
    } else if (e.value == "Đã sử dụng") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.stateFilter === "Đã sử dụng"
      );
      setData(filterdata);
    } else if (e.value == "Bỏ qua") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.stateFilter === "Bỏ qua"
      );
      setData(filterdata);
    } else if (e.value == "Khám răng hàm mặt") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.service === "Khám răng hàm mặt"
      );
      setData(filterdata);
    } else if (e.value == "Khám tai mũi họng") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.service === "Khám tai mũi họng"
      );
      setData(filterdata);
    } else if (e.value == "Khám sản - Phụ khoa") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.service === "Khám sản - Phụ khoa"
      );
      setData(filterdata);
    } else if (e.value == "Kiosk") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.source === "Kiosk"
      );
      setData(filterdata);
    } else if (e.value == "Hệ thống") {
      const filterdata = dataNumberLevel.filter(
        (item) => item.source === "Hệ thống"
      );
      setData(filterdata);
    } else {
  ...
  const optionState = ["Tất cả", "Đang chờ", "Đã sử dụng", "Bỏ qua"];

  const optionSource = ["Tất cả", "Kiosk", "Hệ thống"];
      ...
      <div className="Equiment__container">
        ...
        <div className="ServiceDropdownsSearch">
          <div className="ServiceDropdownsItem1">
            <div className="ServiceDropdownsSearchFirst serviceFix">
              <div className="ServiceDropdownsSearchFirstName">Tên dịch vụ</div>
            ...
            <div className="ServiceDropdownsSearchFirst">
              <div className="ServiceDropdownsSearchFirstName">Tình trạng</div>
              <div className="ServiceDropdowns_1">
                <Dropdown
                  options={optionState}
                  onChange={(e) => handleDropdownValue(e)}
            ...
            <div className="ServiceDropdownsSearchFirst">
              <div className="ServiceDropdownsSearchFirstName">Nguồn cấp</div>
              <div className="ServiceDropdowns_1">
                <Dropdown
        ...
        <div className="ServiceTableDetail">
          <div className="ServiceTableDetailTitles1">
            ...
            <div className="ServiceTableDetailItem" key={index}>
              ...
              <div className="ServiceTableDetailTitles_Item">
                <span onClick={() => handleSend(item)}>
                  {" "}
                  <Link to="/number/detailnumber">Chi tiết</Link>{" "}
                </span>
              </div>
            </div>
          ))}
